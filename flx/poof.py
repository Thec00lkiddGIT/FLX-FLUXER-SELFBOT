...
from flx.config import poof_api_key
from flx.fluxerscript import FlxMessage
from flx.message_attachments import (
    attachment_url,
    download_attachment,
...
def _encode_multipart(fields: list[tuple[str, str]], files: list[tuple[str, str, bytes, str]]) -> tuple[bytes, str]:
    boundary = f"----FlxPoof{uuid.uuid4().hex}"
    parts: list[bytes] = []
...
def poof_from_message(
    message: FlxMessage,
    args: str,
    *,
    fluxer_token: str,
) -> tuple[str, list[tuple[str, bytes]]]:
    attachment = first_image_attachment(message.raw)
    if not attachment:
        raise ValueError("Attach an image to your `!poof` message (JPEG, PNG, or WebP, max 20MB).")

    url = attachment_url(attachment)
    image_bytes, filename, content_type = download_attachment(url, auth_token=fluxer_token)
    ...
    return "\n".join(lines), [(out_name, result_bytes)]
